...
1. Clone this repo.
2. Double-click **`setup.bat`** (clones Pixal3D if needed, creates `venv`, installs CUDA wheels — can take a while).
3. Double-click **`launch_gui.bat`** — opens http://127.0.0.1:7860
...
```
setup.bat                 one-time install (+ clones TencentARC/Pixal3D)
launch_gui.bat            start web UI
launch_gui_lowvram.bat    force Balanced preset env
run_inference.bat         CLI wrapper
webui/                    FastAPI + static UI
scripts/                  env check + Windows CUDA import aliases
...
- First generate downloads multi-GB Hugging Face weights.
